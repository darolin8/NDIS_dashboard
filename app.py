...
def display_ml_insights_section(filtered_df):
    ...
    # Ensure high_severity exists for training
    if "high_severity" not in df_used.columns and "severity" in df_used.columns:
        df_used = df_used.copy()
        df_used["high_severity"] = df_used["severity"].astype(str).isin(["High", "Critical"]).astype(int)
        ...
        if st.button("Train / Refresh", use_container_width=True):
            try:
                st.session_state['trained_models'] = predictive_models_comparison(
                    df_used,
                    target="high_severity",          # ← train for high severity
                    split_strategy="time",
    ...
    # ---------------------------------
    # 🛡️ Diagnose Data Leakage (fixed & scoped)
    # ---------------------------------
    with st.expander("🛡️ Diagnose Data Leakage"):
        if st.button("Run leakage checks"):
            if "reportable" in df_used.columns:
                st.warning("Found 'reportable' column in the data. Leakage guard will drop reportable-like columns from features.")

            if "high_severity" in df_used.columns and "severity" in df_used.columns:
                sev_map = {"Low": 1, "Medium": 2, "High": 3, "Critical": 4}
                tmp = df_used.copy()
                tmp["severity_numeric"] = tmp["severity"].map(sev_map)
                corr = tmp["high_severity"].corr(tmp["severity_numeric"], method="spearman")
                st.write(f"Spearman correlation between high_severity and severity_numeric: **{corr:.3f}** (expected by definition)")

            leaky_like = [c for c in df_used.columns if any(p in c.lower() for p in
            ...
            if leaky_like:
                st.write("Columns that will be **dropped** by the leakage guard (name-based rules):")
                st.code(", ".join(sorted(leaky_like)))
            else:
                st.write("No obviously leaky columns by name.")
